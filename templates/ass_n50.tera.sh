...
cat n50.tsv |
    ...
    tva uniq | # keep the first header line
    tva filter -H --not-blank "N50" |
    tva filter -H --gt "N50:0" | # unfinished downloads
    tva keep-header -- tva join -f url.tsv -k 1 \
    > tmp.tsv
mv tmp.tsv n50.tsv

log_info Calculate N50 not in the list
cat url.tsv |
    tva join -f n50.tsv -k 1 -e |
...
cat n50.tsv |
    tva filter -H --ge "N50:${LEN_N50}" |
    tva filter -H --le "C:${N_CONTIG}" |
    tva filter -H --ge "S:${LEN_SUM}" \
    > n50.pass.tsv
